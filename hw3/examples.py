...
# 加载ImageNet的类别索引
with open("./hw3_CNN-basic/imagenet_class_index.json", "r") as fp:
...
# 加载图像
img = Image.open('./hw3_CNN-basic/elephant.jpg')
img_t = preprocess(img)
print("size is {}".format(img_t.size()))
batch_t = torch.unsqueeze(img_t, 0) #将单张图片转换为batch

vgg16_entire.eval() # 设置为评估模式
print("result is {}".format(vgg16_entire(batch_t)))
...
for idx, layer in enumerate(vgg16_entire.features.children()):
